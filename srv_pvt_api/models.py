...
from app.App_Common import Timer
...
class bytesave_cycle(models.Model):
    ...
    # 0: tháng; 1: năm
    bytesave_time_type = models.ForeignKey(time_types, on_delete=models.CASCADE, null=True)
...
class upgrade_version_type(models.Model):
    yes='1'
    no ='0'
    type = (
        (yes,'ký hợp đồng nâng cấp'),
        (no,'không ký hợp đồng nâng cấp')
    )
    title = models.CharField(choices=type,max_length=100,null=True)

    def create_or_no_update(self):
        item_type = logged_type.objects.filter(title = self)
        if item_type.count() == 0:
            type = logged_type.objects.create(title=self)
            return type
...
class Customers(models.Model):
    ...
    city = models.ForeignKey(citys, on_delete=models.CASCADE, default=None, blank=True, null=True)
    ...
    '''quy mô'''
    scale = models.ForeignKey(employees, on_delete=models.CASCADE, default=None, blank=True, null=True)
    # lĩnh vức
    field = models.IntegerField(default=1)
    # 0: không ký hợp đồng nâng cấp; 1: ký hợp đồng nâng cấp
    is_upgrade_version = models.ForeignKey(upgrade_version_type, on_delete=models.CASCADE, default=None, blank=True, null=True)
    is_del = models.IntegerField(default=0)
    time_create_at = models.IntegerField(default=Timer.get_timestamp_now())
    time_update_at = models.IntegerField(default=Timer.get_timestamp_now())
...
class type_contacts(models.Model):
    code = models.CharField(max_length=250, blank=True,null=True)
    title = models.CharField(max_length=250, blank=True)
...
class Customer_Represent(models.Model):
    ...
    type = models.ForeignKey(type_contacts, on_delete=models.CASCADE, default=None, blank=True, null=True)
    is_del = models.IntegerField(default=0)
    time_create_at = models.IntegerField(default=Timer.get_timestamp_now())
    time_update_at = models.IntegerField(default=Timer.get_timestamp_now())
...
class Metric_Services(models.Model):
    ...
    is_service = models.ForeignKey(services, on_delete=models.CASCADE, default=None, blank=True, null=True)
    # 0: Chưa sử dụng; 1: Đã sử dụng
    status = models.ForeignKey(status_type, on_delete=models.CASCADE, default=None, blank=True, null=True)
...
class logged_type(models.Model):
    yes='1'
    no ='0'
    type = (
        (yes,'duy trì'),
...
class Agents(models.Model):
    ...
    #0: khong
    #1: co duy trì
    is_logged = models.ForeignKey(logged_type, on_delete=models.CASCADE, default=None, blank=True, null=True)
    is_del = models.IntegerField(default=0)
    time_create_at = models.IntegerField(default=Timer.get_timestamp_now())
    time_update_at = models.IntegerField(default=Timer.get_timestamp_now())
...
class folder_type(models.Model):
    file='0'
    folder ='1'
    type = (
        (file,'filde'),
...
class backup_bytesave(models.Model):
    ...
    #0:file; 1:folder
    is_folder = models.ForeignKey(folder_type, on_delete=models.CASCADE, default=None, blank=True, null=True)
